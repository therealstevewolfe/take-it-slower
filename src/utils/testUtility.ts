...
import { AIAssistantOutputAnalyzer, TaskInfo } from '../parsers/aiAssistantOutputAnalyzer';
...
export class TestUtility {
   ...
   */
  public async runTest(assistantType: 'rooCode' | 'cline'): Promise<void> {
    // Load sample outputs
   ...
   */
  private processTaskInfos(taskInfos: TaskInfo[], assistantId: string): void {
    for (const taskInfo of taskInfos) {
      ...
      if (!task) {
        ...
        task = this.taskManager.createTask(
          ...
          taskInfo.name,
          assistantId,
          taskInfo.type || TaskType.Generic,
          taskInfo.parentId
        );
      }
   ...
   */
  private async loadSampleOutputs(assistantType: string): Promise<string[]> {
   ...
   */
  private formatTaskHierarchy(task: Task, level: number): string {
    const indent = '  '.repeat(level);
    let result = `${indent}- ${task.getName()} (${task.getStatus()}`;
    
    if (task.getProgress() > 0 && task.getStatus() === TaskStatus.InProgress) {
